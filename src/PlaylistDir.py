...
class Data_Handler:
    def __init__(self):
        ...
        self.include_subfolders = os.environ.get("include_subfolders", "False").lower()
    ...
    def add_playlist_to_plex(self, subfolder):
        try:
            ...
            enccoded_m3u_path = urllib.parse.quote(m3u_path, safe='')
            url = f"{self.plex_server_ip}/playlists/upload?sectionID={self.plex_library_section_id}&path={enccoded_m3u_path}&X-Plex-Token={self.x_plex_token}"
            response = requests.post(url)
    ...
    def create_playlists(self):
        try:
            ...
            if self.include_subfolders == "true":
                subfolders = [os.path.relpath(os.path.join(root, d), self.parent_folder) for root, dirs, _ in os.walk(self.parent_folder) for d in dirs]
            else:
